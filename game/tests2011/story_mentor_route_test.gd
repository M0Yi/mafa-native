...
func run() -> void:
	...
	app.store.path="/tmp/mentor-visit-routes-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	app.start_character({"id":"mentor_visit","name":"职业拜访","gender":"男","job":"战士"});app.world.hide()
	var legs: Array=[]
	for pair in [["server:merchant:9","server:merchant:41"],["server:merchant:41","server:merchant:9"],["server:merchant:9","server:merchant:46"],["server:merchant:46","server:merchant:9"],["server:merchant:9","server:merchant:44"],["server:merchant:44","server:merchant:9"]]:
		...
		if origin.map!=target.map:
			...
			expect(not path.is_empty() and path.size()<=4,"mentor visit recipient reachable within four crossings "+str(pair))
			for gate in path:
	...
	var report:={"checks":checks,"failures":failures,"legs":legs,"scope":"actual map geometry, landing and live occupancy for mentor visit NPC routes; source neighborhoods are fixtures; doors and NPC approaches use continuous player movement; other actors remain stationary, no combat or physical input"}
	FileAccess.open("res://../artifacts/world-story/mentor-visit-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
