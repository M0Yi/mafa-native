...
func click(button: Control) -> void:
	var guard:=0
	while root.gui_get_focus_owner()!=button and guard<30:
		await press(JOY_BUTTON_DPAD_DOWN);guard+=1
	expect(root.gui_get_focus_owner()==button,"dpad reaches "+button.text)
...
func run() -> void:
	...
	app.store.path="/tmp/controller-sea-travel-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	...
	expect(app.windows.order.back()=="魔龙城传送路线","B restores previous route window")
	await click(find_button(app.form,"前往苍月传送石"))
	...
	expect(app.world.metadata.id=="5" and app.rules.state.gold==1000,"controller ferry pays original fare once")
	...
	var report:={"checks":checks,"failures":failures,"scope":"actual NPCs and landing tiles, viewport joypad guidance/ferry, fare rollback, city and return rules; NPC approach, funds and level are fixtures, continuous world travel pending"}
	FileAccess.open("res://../artifacts/world-story/controller-controller-sea-travel-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
