...
func run() -> void:
	...
	app.store.path="/tmp/old-mine-route-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app)
	for i in range(8):await process_frame
	app.set_process(false);app.start_character({"id":"mine-route","name":"旧矿道探查","job":"战士","gender":"男"});app.world.hide()
	var prepared: Dictionary=app.rules.state.duplicate(true);prepared.quests.story_seal_mine="accepted"
	...
	expect(routes.size()==2,"old mine has explicit two-way reconstructed link")
	for route in routes:
	...
	expect(EditionRules.Story.ready(app.rules.state,EditionRules.Story.quest("story_seal_mine")),"actual crossing records old mine visit")
	var report:={"checks":checks,"failures":failures,"scope":"two-way actual player steps and passage polling; initial doorway and quest fixtures, actors/time not advanced, not full valley journey"}
	FileAccess.open("res://../artifacts/world-story/old-mine-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free()
	for i in range(8):await process_frame
