...
func run() -> void:
	...
	app.store.path="/tmp/sabuk-supply-routes-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	...
	var report:={"checks":checks,"failures":failures,"legs":legs,"scope":"continuous palace to pharmacy to warehouse to town veteran; actual purchases, storage and task delivery; initial position/prerequisite/budget fixtures, stationary actors, no hardware input"}
	FileAccess.open("res://../artifacts/world-story/sabuk-supply-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
