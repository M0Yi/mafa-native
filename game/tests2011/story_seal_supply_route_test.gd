...
func run() -> void:
	...
	app.store.path="/tmp/seal-supply-routes-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	app.start_character({"id":"seal_supply","name":"封魔后勤","gender":"男","job":"战士"});app.world.hide()
	var legs: Array=[]
	for pair in [["server:merchant:141","server:merchant:138"],["server:merchant:138","server:merchant:134"],["server:merchant:134","server:merchant:135"],["server:merchant:135","server:merchant:137"],["server:merchant:137","server:merchant:132"],["server:merchant:132","server:merchant:133"],["server:merchant:133","server:merchant:141"]]:
		...
		if origin.map!=target.map:
			...
			expect(not path.is_empty() and path.size()<=4,"seal supply recipient reachable within four crossings "+str(pair))
			for gate in path:
	...
	var report:={"checks":checks,"failures":failures,"legs":legs,"scope":"actual map geometry, landing and live occupancy for seal supply NPC routes; source neighborhoods are fixtures; doors and NPC approaches use continuous player movement; other actors remain stationary, no combat or physical input"}
	FileAccess.open("res://../artifacts/world-story/seal-supply-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
