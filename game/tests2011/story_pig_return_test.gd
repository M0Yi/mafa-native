...
func run() -> void:
	...
	app.store.path="/tmp/pig-side-return-routes-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	...
	for q in EditionRules.Story.data().quests:
		if q.kind=="石墓侧路调查":
			for objective in q.objectives:
				if objective.type=="visit" and objective.map not in targets:targets.append(objective.map)
	for mid in targets:
	...
	FileAccess.open("res://../artifacts/world-story/pig-side-return-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
