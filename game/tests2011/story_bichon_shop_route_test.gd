...
func run() -> void:
	...
	app.store.path="/tmp/bichon-shops-routes-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	app.start_character({"id":"bichon_shops","name":"比奇后勤","gender":"男","job":"战士"});app.world.hide()
	...
	var legs: Array=[]
	for pair in [["server:merchant:9", "server:merchant:2"], ["server:merchant:2", "server:merchant:3"], ["server:merchant:3", "server:merchant:8"], ["server:merchant:8", "server:merchant:5"], ["server:merchant:5", "server:merchant:14"], ["server:merchant:14", "server:merchant:15"], ["server:merchant:15", "server:merchant:16"], ["server:merchant:16", "server:merchant:9"]]:
		...
		if origin.map!=target.map:
			...
			expect(not path.is_empty() and path.size()<=32,"bichon shop recipient reachable within thirty-two crossings "+str(pair))
			for gate in path:
	...
	var report:={"checks":checks,"failures":failures,"legs":legs,"scope":"actual map geometry, landing and live occupancy for bichon shop NPC routes; source neighborhoods are fixtures; doors and NPC approaches use continuous player movement; other actors remain stationary, no combat or physical input"}
	FileAccess.open("res://../artifacts/world-story/bichon-shops-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
