...
func run() -> void:
	...
	app.store.path="/tmp/jungle-branch-return-routes-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	...
	for mid in ["d12"]:
		...
		for names in [["半兽人","半兽战士"],["僵尸2","僵尸3","僵尸4","僵尸5"]]:
			var reachable: Array=[]
			for point in regional.populations[mid]:
				if regional.spawns[int(point[0])].name not in names:continue
				var target:=Vector2i(point[2],point[3])
				if not app.world.navigation.path(landing,target).is_empty():reachable.append(target)
				if reachable.size()>=4:break
			expect(reachable.size()>=4,"quest population reachable from branch landing "+str(names))
			if not reachable.is_empty():
				expect(app.enter_map(mid,reachable[0]),"load actual quest spawn area")
				app.world.paused=true
				expect(app.world.entities.any(func(e):return e.get("reference_name","") in names),"actual streamed quest monsters present "+str(names))
		app.enter_map(mid,landing);app.world.paused=true
		...
		expect(path.size()==1,"expedition return requires one crossing "+mid)
	...
	for mid in ["11","d12"]:
		for leg in app.resources.connections.by_map[mid]:
			if leg.get("source",{}).get("rule","")!="jungle_branch_investigation_v1":continue
			crossings+=1
			var door:=Vector2i(leg.cell[0],leg.cell[1])
	...
	expect(crossings==2,"one bidirectional branch connection")
	...
	FileAccess.open("res://../artifacts/world-story/jungle-branch-return-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
