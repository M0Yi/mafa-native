...
func run() -> void:
	...
	var quest_id:="story_mongchon_storage"
	...
	expect(app.pending_message.begins_with("已进入") and not "索引" in app.pending_message and not app.resources.world_catalog[giver.map].missing_references.is_empty(),"arrival is player-facing while diagnostic gaps remain recorded")
	...
	expect(app.rules.story_action(q.id,"accept",giver.id,giver.map,app.world.player.cell),"accept fixture")
	expect(app.enter_map(npc.map,at) and app.save_world(),"load warehouse after accepting task")
	expect(app.resources.world_catalog[npc.map].missing_references.is_empty() and not "缺口" in app.pending_message,"warehouse does not inherit previous map gap warning")
	...
	if not app.windows.windows.has("个人仓库"):app.queue_free();await settle();quit(1);return
	await settle()
	...
	if source!=null:
		...
		expect(EditionInventory.find_item(app.rules.state,uid).container=="warehouse" and Story.progress(app.rules.state,q,1)==1,"mouse deposit moves item and counts at money house")
		expect("下一步：在指定仓库取回" in view.material_lines[0].label.text,"task preparation follows actual drag deposit")
		...
		expect(view.material_lines[0].label.text.is_empty(),"task preparation clears after actual withdrawal")
	var report:={"checks":checks,"failures":failures,"scope":"native viewport mouse button/motion events; map, prerequisite and window positions are fixtures; no physical mouse or gamepad"}
