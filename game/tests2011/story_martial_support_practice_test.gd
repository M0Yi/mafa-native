...
func run() -> void:
	...
	app.store.path="/tmp/martial-support-practice-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	for skill in ["halfmoon","flame","armor","groupheal"]:
		var expected_job: String=EditionSkills.DEFINITIONS[skill].job
		var q: Dictionary=Story.quest("story_"+("warrior" if expected_job=="战士" else "tao")+"_"+skill+"_practice")
		for job in ["战士","道士","法师"]:
			app.start_character({"id":skill+job,"name":"法术修习","gender":"男","job":job});app.world.hide()
			var next: Dictionary=app.rules.state.duplicate(true);next.level=60;next.hp=1;next.mp=560
			...
			expect(accepted==(job==expected_job),"practice respects job restriction")
			if job!=expected_job:continue
			...
			if skill=="groupheal":
				var party_state: Dictionary=app.rules.state.duplicate(true);party_state.party=["青禾","远山","轻舟"]
				expect(app.rules.apply(party_state,"support_party_fixture"),"save support party fixture")
				for member in [["near","青禾",301,100],["far","远山",306,100],["dead","轻舟",302,0],["stranger","云游客",303,100]]:
					app.world.entities.append({"id":"support-"+str(member[0]),"kind":"traveler","name":member[1],"cell":[member[2],618],"hp":member[3],"generation":0})
			...
			if skill in ["halfmoon","flame"]:expect(target.hp<10000,"actual martial impact damages eligible nearby target")
			elif skill=="armor":expect(float(app.rules.state.get("armor_until",0))>app.elapsed,"armor practice applies timed defense")
			else:
				expect(app.rules.state.hp>1,"group heal practice restores player health")
				var health: Dictionary=app.rules.state.get("traveler_health",{})
				expect(int(health.get("support-near",{}).get("hp",0))>100,"nearby living party member healed")
				for excluded in ["far","dead","stranger"]:expect(not health.has("support-"+excluded),"group heal excludes "+excluded)
	...
	var report:={"checks":checks,"failures":failures,"scope":"warrior and tao practice quests, viewport mouse task-to-skill opening and current skill selection, real book pickup/use and casts; deterministic book drop, prerequisite, level, targets and delivery location fixtures; not natural loot or complete travel"}
	FileAccess.open("res://../artifacts/world-story/martial-support-practice-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
