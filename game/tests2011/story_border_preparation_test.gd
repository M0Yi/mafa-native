...
func run() -> void:
	...
	app.store.path="/tmp/border-preparation-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	app.start_character({"id":"border_preparation","name":"村中行装","gender":"男","job":"战士"});app.world.hide()
	...
	for pair in [["border:elder","server:merchant:35"],["server:merchant:35","server:merchant:36"],["server:merchant:36","border:elder"]]:
		...
		expect(EditionRules.Story.ready(app.rules.state,EditionRules.Story.quest(quest_id))==(legs.size()>0),"two room visits and conversations required")
		if target.id=="border:elder":expect(app.rules.story_action(quest_id,"submit",target.id,target.map,app.world.player.cell),"submit at walked village elder")
		app.windows.close_all()
		legs.append({"from":pair[0],"to":pair[1],"crossings":path.size()})
	var report:={"checks":checks,"failures":failures,"legs":legs,"scope":"actual map geometry, landing and live occupancy for border bookshop and warehouse NPC routes; one initial source neighborhood and prior quest fixture; continuous three-leg walking, real recipient interaction and delivery rules; other actors remain stationary, no combat or physical input"}
	FileAccess.open("res://../artifacts/world-story/border-preparation-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
