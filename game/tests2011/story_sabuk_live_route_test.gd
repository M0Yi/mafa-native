...
func run() -> void:
	...
	app.store.path="/tmp/sabuk-live-routes-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	app.start_character({"id":"sabuk_story","name":"城堡见闻","gender":"男","job":"战士"});app.world.hide()
	var prepared: Dictionary=app.rules.state.duplicate(true);prepared.quests.story_guild_supplies="done"
	...
	var legs: Array=[]
	for pair in [["server:merchant:80","server:npc:1"],["server:npc:1","server:merchant:80"]]:
		...
		var quest_id: String="story_sabuk_introduction" if legs.is_empty() else "story_sabuk_return"
	...
	var report:={"simulated_seconds":simulated_frames/60.0,"moving_entities":moved.size(),"minimum_hp":minimum_hp,"checks":checks,"failures":failures,"legs":legs,"scope":"actual map geometry, landing and live occupancy for sabuk NPC routes; one initial source neighborhood and prior quest fixture; continuous outbound/return walking, real recipient interaction and delivery rules; all actors and gameplay timers update at simulated 60 Hz; no boosted health, no physical input; successful travel does not prove a populated siege"}
	FileAccess.open("res://../artifacts/world-story/sabuk-live-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
