...
func run() -> void:
	...
	app.store.path="/tmp/leiyan-return-routes-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	app.start_character({"id":"return-routes","name":"归程核验","gender":"男","job":"战士"});app.world.hide();app.world.paused=true
	var npc: Dictionary=app.rules.story_npc("server:merchant:162")
	...
	FileAccess.open("res://../artifacts/world-story/leiyan-return-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
