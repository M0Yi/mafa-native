...
func run() -> void:
	...
	app.store.path="/tmp/border-live-routes-"+Crypto.new().generate_random_bytes(8).hex_encode()+".sqlite";root.add_child(app);await settle();app.set_process(false)
	app.start_character({"id":"border_live","name":"村内行程","gender":"男","job":"战士"});app.world.hide()
	var prepared: Dictionary=app.rules.state.duplicate(true);prepared.quests.nv_patrol="done";prepared.novice={"chickens":0,"patrol":true}
	...
	var legs: Array=[]
	for pair in [["border:elder","server:merchant:35"],["server:merchant:35","server:merchant:36"],["server:merchant:36","border:elder"]]:
		...
		var quest_id: String="story_border_preparation"
		...
		if legs.size()>0:expect(EditionRules.Story.ready(app.rules.state,EditionRules.Story.quest(quest_id)),"actual recipient interaction makes report ready")
		if target.id=="border:elder":expect(app.rules.story_action(quest_id,"submit",target.id,target.map,app.world.player.cell),"submit at walked recipient position")
		app.windows.close_all()
		legs.append({"from":pair[0],"to":pair[1],"crossings":path.size()})
	expect(not moved.is_empty(),"live movement was observed during route")
	var report:={"simulated_seconds":simulated_frames/60.0,"moving_entities":moved.size(),"minimum_hp":minimum_hp,"checks":checks,"failures":failures,"legs":legs,"scope":"actual map geometry, landing and live occupancy for village bookshop and warehouse routes; one initial source neighborhood and prior quest fixture; continuous three-leg walking, real recipient interaction and delivery rules; all actors and gameplay timers update at simulated 60 Hz; no boosted health, no physical input; successful travel does not prove a full dynamic world"}
	FileAccess.open("res://../artifacts/world-story/border-live-route-tests.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print(JSON.stringify(report));app.queue_free();await settle();quit(0 if failures.is_empty() else 1)
