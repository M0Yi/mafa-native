...
func setup(host,container: String,embedded:=false) -> void:
	...
	sort.pressed.connect(func():
		...
		if app.rules.state.hp<=0:app.pending_message="死亡状态不能使用物品";return
		if app.world.paused:app.pending_message="请继续游戏后操作物品";return
		app.rules.inventory_action("sort",{});app.pending_message=app.rules.message)
	...
	use.pressed.connect(func():
		...
		if container_id=="warehouse":
			if not app.warehouse_access_allowed():return
			if app.world.paused:app.pending_message="请继续游戏后操作物品";return
			app.rules.inventory_action("move",{"uid":app.selected_item_uid,"container":"inventory","slot":EditionInventory.free_slot(app.rules.state.items,"inventory")})
			app.pending_message=app.rules.message
		else:app.gameplay.use_item_uid(app.selected_item_uid))
...
func split_prompt() -> void:
	if not is_inside_tree() or is_queued_for_deletion():return
	if app.mode!="game" or app.rules.state.hp<=0 or app.windows.has_modal():return
	if container_id=="warehouse" and not app.warehouse_access_allowed():return
	if app.world.paused:app.pending_message="请继续游戏后操作物品";return
	...
	for i in range(candidates.size()):
		...
		choice.add_item(EditionRules.ITEMS[item.type].name+" ×%d · 格%d"%[int(item.count),int(item.slot)+1])
		if item.uid==app.selected_item_uid:selected_index=i
	choice.select(selected_index)
	var count:=SpinBox.new();count.min_value=1;count.max_value=candidates[selected_index].count-1;count.value=1;layout.add_child(count)
	choice.item_selected.connect(func(i):count.max_value=candidates[i].count-1)
	var error:=Label.new();error.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART;layout.add_child(error)
	dialog.dialog_hide_on_ok=false
	dialog.confirmed.connect(func():
		if not is_inside_tree() or is_queued_for_deletion() or not dialog.is_inside_tree() or dialog.is_queued_for_deletion() or not dialog.visible:return
		if app.mode!="game" or app.rules.state.hp<=0:error.text="请进入游戏并复活后操作物品";return
		if container_id=="warehouse" and not app.warehouse_access_allowed():error.text=app.pending_message;return
		if app.world.paused:error.text="请继续游戏后操作物品";app.pending_message=error.text;return
		var uid: String=candidates[choice.selected].uid
		var current:=EditionInventory.find_item(app.rules.state,uid)
		if current.get("container","")!=container_id:
			error.text="所选物品已移动或不存在，请关闭后重新选择";app.pending_message=error.text;return
		var ok: bool=app.rules.inventory_action("split",{"uid":uid,"count":int(count.value)})
		app.pending_message=app.rules.message
		if ok:dialog.queue_free()
		else:error.text=app.rules.message)
