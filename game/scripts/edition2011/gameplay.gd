...
func bind_skill(id: String,key: int) -> void:
	...
	next.skill_keys[str(key)]=id;app.rules.apply(next,"bind_skill");say("已更新自动技能顺序")
...
func cast(id: String) -> bool:
	...
	if skill.range>0:
		...
		if id!="halfmoon" and skill.get("kind","")!="around":
			...
			match skill.get("kind",""):
				"area":targets=targets.filter(func(e):return maxi(absi(int(e.cell[0])-int(chosen.cell[0])),absi(int(e.cell[1])-int(chosen.cell[1])))<=1)
				"line":
	...
	if id=="groupheal":
		...
		for e in app.world.entities:
			...
			next.traveler_health[e.id]={"hp":mini(250,int(e.get("hp",250))+30+int(next.level)*2),"generation":int(e.get("generation",0)),"respawn":0}
	...
	spell_events.append({"at":app.elapsed+0.12,"map":app.world.metadata.id,"skill":id,"stage":1})
...
func show_utilities() -> void:
	...
	for row in [["strength","力量：每点攻击 +1"],["vitality","体力：每点生命上限 +5"],["spirit","精神：每点伤害技能威力 +1"]]:
		app.button(row[1]+" · 已加 %d"%int(app.rules.state.get("attributes",{}).get(row[0],0)),func():app.rules.allocate(row[0]);say(app.rules.message);show_utilities())
	for type in ["return_stone","reset_stone"]:
		app.button("购买 %s · %d 金币"%[EditionRules.ITEMS[type].name,EditionRules.ITEMS[type].price],func():
			if app.rules.state.hp>0:app.rules.shop(type,1);say(app.rules.message))
func revive_after_wait() -> bool:
	...
	if not app.enter_map("0",EditionVillage.SPAWN,false):
		app.enter_map(origin,cell,false);app.death_return_at=due;return false
...
func ensure_death() -> void:
	...
	active_assist=false;pending_pickup="";spell_events.clear();visuals.clear();queue_redraw();app.pending_attack.clear();app.world.player.route.clear()
	...
	resume.pressed.connect(func():app.windows.modal.hide();app.windows.pause_reason="";app.world.paused=false)
	var back:=Button.new();back.text="保存并返回角色选择";box.add_child(back)
...
func auto_pickup_underfoot(cfg: Dictionary) -> void:
	if not cfg.get("pickup",false) or not available():return
	for loot in app.rules.state.get("ground_loot",[]).duplicate(true):
		...
		if not pickup(loot):assist_left=2;return
...
func approach_target(entity: Dictionary) -> void:
	...
	for direction in ClassicNavigation.DIRECTIONS:
		...
		if not app.world.navigation.can_step(candidate,target):continue
		var route: Array[Vector2i]=app.world.navigation.path(app.world.player.cell,candidate)
		if route.is_empty() or target in route:continue
		if best.is_empty() or route.size()<best.size():best=route
	if not best.is_empty():best.pop_front();app.world.player.route=best
...
func hit_traveler(entity: Dictionary,attack: Dictionary) -> void:
	...
	var defense_key: String="mac" if attack.get("magic_attack",false) else "ac"
...
func play_skill_stage(id: String,stage: int) -> void:
	...
	# Empty stages in sound.lst intentionally stay silent.
